...
`horizontal-sizing.ts` verifies the shared sizing example in both editor integrations: inspector column changes, canvas/preview bounding boxes, Undo/Redo, and JSON Apply/Revert. Renderer-package browser tests cover wrapping and reactive visibility for all three UI libraries.
