...
export const parseMode = (value: boolean | string): ThemeMode => {
  if (typeof value === 'boolean') {
...
export const normalizeSchema = <T extends Record<string, unknown> | undefined>(schema: T): T => {
  if (!schema || schema.$id) {
    return schema;
  }

  return { ...schema, $id: '/' } as T;
};
