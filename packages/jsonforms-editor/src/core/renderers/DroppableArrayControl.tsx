...
interface DefaultArrayLayoutProps {
  isOver: boolean;
}

const DefaultArrayLayout = styled(Typography, {
  shouldForwardProp: (prop) => prop !== 'isOver',
})<DefaultArrayLayoutProps>(({ isOver }) => ({
  padding: 10,
...
const DroppableArrayControl: React.FC<DroppableArrayControlProps> = ({
  ...
  const [{ isOver, uiSchemaElement }, drop] = useDrop<
    { type: string; uiSchemaElement: EditorControl },
    void,
    { isOver: boolean; uiSchemaElement?: EditorControl }
  >({
  ...
  if (!uischema.options?.detail) {
    return (
      <DefaultArrayLayout ref={drop} isOver={isOver}>
