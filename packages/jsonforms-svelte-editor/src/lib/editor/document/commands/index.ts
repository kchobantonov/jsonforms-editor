...
import { jsonSchemaFields } from "../../inspector/advanced.js";
...
export function updateProperties(
  ...
  if ("layoutType" in data && ["VerticalLayout", "HorizontalLayout"].includes(node.type)) {
    if (!["VerticalLayout", "HorizontalLayout"].includes(String(data.layoutType))) throw new Error("Choose a layout type.");
    node.type = String(data.layoutType);
  }
  let changedChoiceSchema = false;
  if (node.type === "Control") for (const [key, keyword] of Object.entries(jsonSchemaFields)) {
    if (!(key in data)) continue;
    const value = data[key] === undefined || data[key] === "" ? undefined : parseSchemaValue(key, String(data[key])) as JsonValue;
    if (JSON.stringify(schema[keyword]) === JSON.stringify(value)) continue;
    if (["enum", "oneOf", "items"].includes(keyword)) changedChoiceSchema = true;
    if (value === undefined) delete schema[keyword]; else schema[keyword] = value;
  }
  ...
  if (node.type === "Control" && "itemTypes" in data) {
    const types = data.itemTypes;
    if (types !== undefined && (!Array.isArray(types) || !types.length || types.some(type => !["string", "number", "integer", "boolean", "object", "array", "null"].includes(type))))
      throw new Error("Choose at least one schema type.");
    if (schema.items !== undefined && (typeof schema.items !== "object" || Array.isArray(schema.items) || schema.items === null))
      throw new Error("Edit boolean or tuple items in Advanced.");
    const items = object(schema.items);
    const previousType = JSON.stringify(items.type);
    if (types === undefined) delete items.type;
    else {
      const unique = [...new Set(types as string[])];
      items.type = unique.length === 1 ? unique[0] : unique;
    }
    if (schema.items !== undefined || types !== undefined) schema.items = items;
    if (previousType !== JSON.stringify(items.type)) changedChoiceSchema = true;
  }
  ...
  if ("columns" in data) {
    if (data.columns !== undefined && data.columns !== null && data.columns !== "auto" && !(typeof data.columns === "number" && Number.isInteger(data.columns) && data.columns >= 2 && data.columns <= 16)) throw new Error("Columns must be Auto or an integer from 2 through 16.");
    const options = object(node.options);
    if (typeof data.columns === "number") options.columns = data.columns;
    else delete options.columns;
    if (Object.keys(options).length) node.options = options;
    else delete node.options;
  }
  applyChoiceProperties(schema, node, changedChoiceSchema ? { choiceWidget: data.choiceWidget } : data);
