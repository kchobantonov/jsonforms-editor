...
export function createSession(
  ...
  let explicitSelection = $state(false);
  ...
  return {
    ...
    openRule(path: number[]) { if (ruleDraft) return; selected = path; explicitSelection = true; schemaSelection = undefined; ruleFocus++; },
    ...
    editSchema(edit: SchemaEdit) {
      ...
      schemaSelection = edit.action === "rename"
        ? edit.pointer.slice(0, edit.pointer.lastIndexOf("/") + 1) + escapePointer(edit.name!.trim())
        : undefined;
      const placement = schemaSelection ? schemaOccurrences(designRoot(next)).find(item => item.scope === schemaSelection) : undefined;
      selected = placement?.path ?? [];
      explicitSelection = !!placement;
      commit(next);
    },
    ...
    resizeElement(id: string, columns: number) {
      if (locked || dragging) return;
      attempt(() => {
        const path = pathFor(designRoot(document), id);
        if (!path?.length || at(designRoot(document), path.slice(0, -1)).type !== "HorizontalLayout") return;
        const next = updateProperties(document, path, { columns });
        selected = path;
        explicitSelection = true;
        schemaSelection = undefined;
        commit(next);
