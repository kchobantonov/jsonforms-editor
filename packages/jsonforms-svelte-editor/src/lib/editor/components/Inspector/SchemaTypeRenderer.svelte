...
{#if binding.control.visible}
  <Field.Field class="schema-type-editor" data-invalid={!!binding.control.errors}>
    <Field.Label for={id}>{binding.control.label || i18n.t("Type")}</Field.Label>
    {#if multiple && arrayValue}
      ...
      >
        <Select.Trigger
          {id}
          class="w-full"
          aria-label={binding.control.label || i18n.t("Type")}
    ...
    {/if}
    {#if binding.control.description}<p class="muted">{binding.control.description}</p>{/if}
