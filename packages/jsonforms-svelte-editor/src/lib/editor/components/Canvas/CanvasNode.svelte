...
<div
  ...
  class="design-node"
  class:selected={session.hasSelection && !session.unplacedSchemaSelection &&
    path !== undefined &&
...
>
  ...
  {#if container}
    ...
    <Button
      ...
      >{node.type === "Categorization"
        ? i18n.t("Drag a category here to add a tab")
        : i18n.t("Drag components or fields into this layout")}</Button
    >
