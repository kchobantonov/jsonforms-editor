...
const node = {
  ...
  properties: {
    ...
    options: {
      type: "object",
      properties: { columns: { anyOf: [{ type: "integer", minimum: 2, maximum: 16 }, { enum: ["auto", null] }], description: "Width within a 16-column HorizontalLayout row. Auto shares remaining space." }, multi: { type: "boolean" }, variant: { type: "string" }, height: { type: "number", minimum: 0, default: 32 }, src: { type: "string" }, alt: { type: "string" }, format: { type: "string" } },
    },
    rule: { type: "object" },
  },
  required: ["type"],
