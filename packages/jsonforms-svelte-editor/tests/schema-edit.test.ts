...
const deletedDefinition = editSchema(doc, { action: "delete", pointer: "#/definitions/Address" });
assert.equal((deletedDefinition.schema as any).definitions.Address, undefined);
assert.equal((deletedDefinition.schema as any).properties.address, undefined);
doc = editSchema(doc, {
  action: "rename",
  pointer: "#/definitions/Address",
  name: "Location",
...
const deletedField = editSchema(doc, {
  action: "delete", pointer: "#/properties/people/items/properties/first~1name",
});
assert.deepEqual(deletedField.uischema?.elements, []);
assert.deepEqual(deletedField.data, { people: [{}] });
