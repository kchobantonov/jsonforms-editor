...
Run `pnpm build:renderers` once first (and after changing the sibling renderer libraries). The demo command builds both editor packages before starting Vite. After editing package sources, rebuild with `pnpm editor:build` and reload. Build the production demo with `pnpm editor:demo:build`, then serve it with `pnpm editor:demo:preview`.

The demo's Example selector uses JSON fixtures from `@jsonforms/examples`. Source acquisition stays in the host; the editor performs no form-resource loading or saving.
