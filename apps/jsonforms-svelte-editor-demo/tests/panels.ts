...
try {
  for (const integration of ["webcomponent", "native"]) {
    ...
    await dragPalette(page, "Label");
    await dragPalette(page, "Button");
    ...
    await page.locator('[aria-label="Components"].palette-locked').waitFor();
    assert.equal(
      await page
        .locator(
          '[aria-label="Components"] .drag-item[aria-label="text"] .palette-row',
        )
    ...
    await page.getByRole("radio", { name: "Design", exact: true }).click();
    await page
      .locator('[aria-label="Components"]:not(.palette-locked)')
      .waitFor();
    assert.equal(
      await page
        .locator(
