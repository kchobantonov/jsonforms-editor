  ...
  for (const integration of ["webcomponent", "native"]) {
    ...
    assert.equal(
      await inspector
        ...
        .count(),
      0,
      "unplaced fields expose schema properties without placement-specific UI options",
    );
    ...
    await page
      .locator(
        ".design-node.selected > .node-heading .canvas-node-actions button",
      )
      .click();
    await tree.getByRole("button", { name: "name", exact: true }).click();
    await page
