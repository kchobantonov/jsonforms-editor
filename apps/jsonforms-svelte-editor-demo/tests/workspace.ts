...
try {
  for (const integration of ["webcomponent", "native"]) {
    ...
    const source = page.locator('[aria-label="Components"] .drag-item[aria-label="textarea"]');
    const target = page.locator("[data-drop-target]").first();
    async function drag(source: Locator, target: Locator) {
      await source.scrollIntoViewIfNeeded();
      await page.locator(".designer-pane").evaluate(element => { element.scrollTop = 0; });
    ...
    await drag(
      page.getByRole("tree", { name: "Schema", exact: true }).getByRole("button", { name: "firstName", exact: true }),
      target,
    );
    await page.waitForFunction(() =>
      window.changes
    ...
    await drag(
      page.locator('[aria-label="Components"] .drag-item[aria-label="textarea"]'),
