...
try {
  for (const integration of ["webcomponent", "native"]) {
    ...
    assert.equal(
      ...
      beforePattern,
      "invalid regex does not corrupt the schema",
    );
    await inspector
      .getByRole("textbox", { name: "Pattern", exact: true })
      .fill("^A");
    await page.waitForFunction(
      () =>
        window.changes.at(-1)?.document.schema.properties.firstName.pattern ===
        "^A",
    );
    const schemaSection = inspector.getByRole("button", {
      name: "Field",
      exact: true,
    });
    assert.equal(await schemaSection.getAttribute("aria-expanded"), "true");
    await inspector.getByRole("button", { name: "Type", exact: true }).click();
    await page
      .getByRole("listbox")
    ...
    await page.waitForFunction(
      () =>
        window.changes.at(-1)?.document.schema.properties.firstName.type ===
        "integer",
    );
    assert.equal(await schemaSection.getAttribute("aria-expanded"), "true");
    assert.equal(
    ...
    const generalCard = inspector.locator('[data-slot="card"]').filter({
