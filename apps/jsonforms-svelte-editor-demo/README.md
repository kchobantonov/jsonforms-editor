...
Browser regression: run `pnpm build:test`, serve the resulting demo on port 4178, then run `pnpm --filter jsonforms-svelte-editor-demo test:browser`. Override `EDITOR_DEMO_URL` if necessary. The test uses Chromium through Playwright.
