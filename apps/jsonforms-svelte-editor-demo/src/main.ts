...
import "./style.css";
// Use the public registry API, never another checkout's private source files.
...
// Regression fixtures are excluded from normal production builds.
if (import.meta.env.MODE === "test") {
  const { regressionExamples } = await import("../tests/fixtures");
  for (const [name, form] of regressionExamples) examples.set(name, form);
}
await import(
  /* @vite-ignore */ new URL(
